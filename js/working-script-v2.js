...
console.log('Working script v2 starting...');
    ...
    try {
        ...
        container.innerHTML = rankings.map(r => `
            <div class="ranking-card">
                <div class="ranking-header clickable-area" onclick="toggleCharacteristicDetail('${r.ticker}')">
                    <div class="rank-info">
                ...
                <div class="characteristics clickable-area" onclick="toggleCharacteristicDetail('${r.ticker}')">
                ...
                <div id="detail-${r.ticker}" class="characteristic-detail" style="display: none;" onclick="event.stopPropagation()">
                    ...
                    ${r.characteristics.map((c, i) => `
                        <div class="characteristic-feedback-item">
                            ...
                            <div class="feedback-slider-container" onclick="event.stopPropagation()">
                                ...
                                <div class="slider-wrapper">
                                    ...
                                    <input type="range" class="feedback-slider" 
                                           ...
                                           oninput="updateSliderValue('${r.ticker}', ${i})"
                    ...
                    `).join('')}
                    <button class="btn-primary" onclick="submitCharacteristicFeedback('${r.ticker}'); event.stopPropagation();">
                        Submit Feedback
                    </button>
                </div>
            </div>
...
// Initialize other sections with simpler version
async function loadPositions(type = 'long') {
    ...
    try {
        ...
        console.log('Positions response:', response.status, response.statusText);
        ...
        // Choose between table or cards based on screen size
        const useCards = window.innerWidth < 1200;
        ...
        if (useCards) {
            ...
            container.innerHTML = `
                <div class="positions-grid">
                    ${positions.map(p => `
                        <div class="position-card ${p.return_pct > 0 ? 'positive-card' : 'negative-card'}">
                            <div class="position-header">
                                <span class="position-ticker">${p.ticker}</span>
                                <span class="position-sector">${p.sector || 'N/A'}</span>
                            </div>
                            <div class="position-metrics">
                                <div class="metric-row">
                                    <span class="metric-label">Entry</span>
                                    <span class="metric-value">$${(p.entry_price || 0).toFixed(2)}</span>
                                </div>
                                <div class="metric-row">
                                    <span class="metric-label">Current</span>
                                    <span class="metric-value">$${(p.live_price || 0).toFixed(2)}</span>
                                </div>
                                <div class="metric-row">
                                    <span class="metric-label">Return</span>
                                    <span class="metric-value ${p.return_pct > 0 ? 'positive' : 'negative'}">
                                        ${p.return_pct > 0 ? '+' : ''}${(p.return_pct || 0).toFixed(2)}%
                                    </span>
                                </div>
                                <div class="metric-row">
                                    <span class="metric-label">vs SPY</span>
                                    <span class="metric-value ${p.return_pct > p.spy_benchmark ? 'positive' : 'negative'}">
                                        ${(p.return_pct - p.spy_benchmark).toFixed(2)}%
                                    </span>
                                </div>
                            </div>
                        </div>
            ...
            // Enhanced table view for larger screens
            container.innerHTML = `
                        ...
                        <tbody>
                            ${positions.map(p => `
                                <tr>
                                    ...
                                    <td class="sector-cell">${p.sector || 'N/A'}</td>
                                    <td>$${(p.entry_price || 0).toFixed(2)}</td>
                                    <td>$${(p.live_price || 0).toFixed(2)}</td>
                                    <td class="${p.return_pct > 0 ? 'positive' : 'negative'}">
                                        ${p.return_pct > 0 ? '+' : ''}${(p.return_pct || 0).toFixed(2)}%
                                    </td>
                                    <td class="${p.return_pct > p.spy_benchmark ? 'positive' : 'negative'}">
                                        ${p.return_pct > p.spy_benchmark ? '↑' : '↓'} ${Math.abs(p.return_pct - p.spy_benchmark).toFixed(2)}%
                                    </td>
                                </tr>
                            `).join('')}
                        </tbody>
        ...
        try {
            const debugResponse = await fetch('http://localhost:8001/positions/long?year=2025');
...
document.addEventListener('DOMContentLoaded', function() {
    // Load positions after a short delay to ensure everything is ready
    setTimeout(() => {
        loadPositions('long');
        loadPerformance();
        loadActiveModels();
...
console.log('Working script v2 loaded successfully');